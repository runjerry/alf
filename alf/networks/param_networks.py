...
class ParamConvNet(Network):
    ...
    def set_parameters(self, theta, reinitialize=False):
        ...
        assert (theta.ndim == 2 and theta.shape[1] == self.param_length), (
            "Input theta has wrong shape %s. Expecting shape (n, %d)" %
...
class CriticDistributionParamNetwork(Network):
    def __init__(self,
                 ...
                 observation_fc_layer_params=((32, True), ),
                 action_fc_layer_params=((32, True), ),
                 joint_fc_layer_params=((64, True), ),
                 activation=torch.relu_,
                 kernel_initializer=None,
                 deterministic=False,
        ...
        if deterministic:
            self._joint_encoder = ParamNetwork(
                ...
                activation=activation,
                kernel_initializer=kernel_initializer,
                last_layer_param=(output_tensor_spec.numel, True),
