...
class SimpleMLP(Network):
    ...
    def compute_jac_diag(self, inputs, hidden_neurons):
        """Compute diagonals of the input-output jacobian. """

        inputs = inputs.squeeze()
        hidden_neurons = hidden_neurons.squeeze()

        assert inputs.shape[-1] == self._input_size, \
            ("inputs should has shape {}!".format(self._input_size))

        assert hidden_neurons.shape[-1] == self._hidden_layer_size, \
            ("hidden_neurons should has shape {}!".format(self._hidden_layer_size))

        mask = (hidden_neurons > 0).float()  # [B, d]
        D = self._decoder.weight.data  # [n, d]
        E = self._encoder.weight.data  # [d, n]
        jac_diag = torch.einsum('ik,bk,ki->bi', D, mask, E)  # [B, n]
