...
class HyperNetwork(Generator):
    ...
    def __init__(self,
        ...
        elif par_vi == 'fisher-ns':
            self._grad_func = self.fisher_grad
            disc_fc_params = (256, 256)
            self._disc_net = EncodingNetwork(
                TensorSpec(shape=(gen_output_dim, )),
                conv_layer_params=None,
                fc_layer_params=disc_fc_params,
                last_layer_size=gen_output_dim,
                last_activation=math.ops.identity)

        else:
            assert ValueError("par_vi only supports \"gfsf\", \"svgd\" \"fisher-ns\"")
